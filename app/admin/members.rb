ActiveAdmin.register Member do
  includes :borrowings,:liked_categories,:favorite_author, borrowings: :book,reservations: :book
  permit_params :name,:email,:phone,:bio,:birth_date,:favorite_author_id,:password,:password_confirmation,
  liked_category_ids: []
  index do
    selectable_column
    id_column
    ...
    column :phone
    column :birth_date
    column("Active Borrowings") {|m| m.borrowings.borrowed.count}
    column :created_at
    actions
  end
  filter :name
  ...
  show do 
    attributes_table do
      ...
      row("Interested Categories") {|m| m.liked_categories.map(&:name).join(", ")}
      row :created_at
      row :updated_at
    end
    panel "Current Borrowings" do
      table_for member.borrowings.borrowed.order(due_date: :asc).limit(10) do
    ...
    panel "Reservations" do
      table_for member.reservations.order(created_at: :desc) do
